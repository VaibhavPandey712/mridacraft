  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload: Record<string, unknown> = {
        ...
        shortDescription: draft.shortDescription || draft.description.slice(0, 140),
      ...
      if (draft.discountPrice) payload["discountPrice"] = Number(draft.discountPrice);

      if (editing) {
        return updateProduct(editing.id, payload, files.length ? files : undefined);
      }
      return createProduct(
        payload as Omit<Parameters<typeof createProduct>[0], "images">,
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button variant="clay" onClick={openCreate}>
          ...
          <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
            ...
            <div className="grid gap-4 py-2 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="p-technique">Technique</Label>
                <Input
                  id="p-technique"
              ...
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input id="p-care" value={draft.care} onChange={(e) => setDraft({ ...draft, care: e.target.value })} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="p-images">
                  Images {editing ? "(leave empty to keep current images)" : ""}
                </Label>
                <Input
                  id="p-images"
                  type="file"
              ...
              <div className="flex items-center gap-3 sm:col-span-2">
                ...
                />
                <Label>Show on homepage as featured</Label>
              </div>
            </div>
            ...
            >
              {saveMutation.isPending ? <Loader2 className="animate-spin" /> : editing ? "Save changes" : "Create product"}
            </Button>
          </DialogContent>
        </Dialog>
      </div>
              ...
              {(products ?? []).map((product) => (
                <TableRow key={product.id}>
                  <TableCell className="flex items-center gap-3">
                    <img src={product.images[0]} alt={product.name} className="size-10 rounded-sm object-cover" />
                  ...
                  <TableCell className="text-right">
                    <Button size="icon" variant="ghost" onClick={() => openEdit(product)} aria-label="Edit">
